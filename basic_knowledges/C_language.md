...
*   标准库不安全语意：gets()
    * gets()把超过缓冲区的字符继续写入堆栈，覆盖了原先内容，可以修改堆栈中的过程活动记录，改变函数返回地址
      * > ？？？（堆栈的结构是什么样的，继续写入堆栈会怎么样？数据结构！
      > 2024年3月1日14:06:42: 堆栈是地址由高到低生长的一块内存区域，LDRFD/STRFD操作，溢出会覆盖堆栈下更低地址的位置，可能是之后调用函数的堆栈帧，所以那个函数调用之后返回地址就被修改了

      > 2024年1月8日08:20:01：这不是数据结构要涉及的内容，可能跟操作系统相关？？关于操作系统如何运行程序

      > 2024年3月1日14:11:49: 对的，跟汇编怎么处理函数调用有关，跟ARM汇编操作单片机有关
#### 参数解析，空格，函数返回局部变量
...
* 结构中允许填充字段，冒号+位长
  ...
  > 2024年3月1日14:19:28: 可以用来表示CPU或者外设寄存器的特定位，比如可以给CPSR的低四位给一个名字
* 与结构相关的参数传参问题
